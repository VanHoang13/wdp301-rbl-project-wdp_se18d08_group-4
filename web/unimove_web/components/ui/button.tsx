   ...
   "disabled:pointer-events-none disabled:opacity-50 select-none cursor-pointer"],
  {
    variants: {
      variant: {
        ...
        "gradient-c": "bg-gradient-to-br from-accent to-accent-secondary text-white shadow-md",
        "gradient-p": "bg-gradient-to-br from-green-700 to-green-500 text-white shadow-md",
      },
      size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading, children, style, ...props }, ref) => {
    ...
    const vs: React.CSSProperties =
      (!variant || variant === "default") ? { backgroundColor: "var(--primary)" } :
