  ...
  return (
    <Ctx.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-[9999] flex flex-col gap-2 w-full max-w-sm px-4 pointer-events-none">
        {toasts.map((t) => (
          <div key={t.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl shadow-xl animate-slide-up border pointer-events-auto"
            style={{ backgroundColor: colors[t.type].bg, color: colors[t.type].text, borderColor: colors[t.type].text + "33" }}>
            {icons[t.type]}
