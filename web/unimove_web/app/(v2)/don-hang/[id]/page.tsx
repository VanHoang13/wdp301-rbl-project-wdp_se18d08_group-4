  ...
  return (
    <div className="pb-6">
      <div className="px-4 pt-4 pb-4 flex items-center gap-3">
        <Link href="/don-hang" className="p-2 rounded-xl" style={{ backgroundColor:"var(--card)", border:"1px solid var(--border)" }}><ArrowLeft size={20} style={{ color:"var(--text)" }}/></Link>
      ...
      {loading?<div className="px-4 space-y-4">{[1,2,3].map(i=><Skeleton key={i} className="h-32 rounded-2xl"/>)}</div>
      :!order?<div className="text-center py-16"><p style={{ color:"var(--muted)" }}>Không tìm thấy đơn hàng</p><Link href="/don-hang"><Button variant="outline" className="mt-4">Quay lại</Button></Link></div>
