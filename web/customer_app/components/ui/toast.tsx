...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const colors: Record<ToastType, { bg: string; text: string; border: string }> = {
    ...
    error: { bg: "var(--error-tint)", text: "var(--error)", border: "var(--error)" },
    info: { bg: "var(--primary-tint)", text: "var(--primary)", border: "var(--primary)" },
  };
      ...
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-[9999] flex flex-col gap-2 w-full max-w-sm px-4">
        {toasts.map((t) => {
          ...
          return (
            <div
              key={t.id}
              className={cn("flex items-center gap-3 px-4 py-3 rounded-2xl shadow-lg animate-slide-up border")}
              style={{ backgroundColor: c.bg, color: c.text, borderColor: c.border + "44" }}
            >
              {icons[t.type]}
