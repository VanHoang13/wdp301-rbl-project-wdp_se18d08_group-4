...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (form.newPassword !== form.confirmPassword) { toast("Mật khẩu không khớp", "error"); return; }
    if (form.newPassword.length < 6) { toast("Mật khẩu mới tối thiểu 6 ký tự", "error"); return; }
    setLoading(true);
    try {
      const res = await providerAuthApi.changePassword({ currentPassword: form.currentPassword, newPassword: form.newPassword });
